...
def get_matched_coordinates(temp_img, map_img, count=''):
    """
    ...
    # result image path
    cv2.imwrite(os.path.join(args.save_dir, 'result'+str(count)+'.png'), img3)

    return dst


if 1: # for matching same diff images
    imgs=[]
    img_names = [ str(i)+'.JPG' for i in range(2,3) ]
    for img in img_names:
        map_img_gray = cv2.imread(img,0)
        #im2 = cv2.resize(im,(512,512))
        map_img_eq = cv2.equalizeHist(map_img_gray)
        imgs.append(map_img_eq)

    temp_img_gray = cv2.imread('2_mask.jpg',0)
    ...
    for im in imgs:
        try:
            coords = get_matched_coordinates(temp_img_eq, im, count)
            print(count,'done!')
        except e:
            print(count,e)
        count+=1
...
if 0: # for matching same class images
    PATH = r'../dataset/C - 5002/F/'
    # read images
    img_names = glob(PATH+'**')
    imgs=[]
    for img in img_names:
