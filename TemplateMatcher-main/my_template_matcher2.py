...
if "I am main":
    ...
    for folder in sorted(glob('../dataset/*'),key=comp_map):
        ...
        img = glob('../dataset/'+label + '/' + name + '/*')[0]
        cnt=1
        map_img_gray = cv2.imread(img,0)
        #im2 = cv2.resize(im,(512,512))
        map_img_eq = cv2.equalizeHist(map_img_gray)
        imgs.append((label,map_img_eq))

    #### create dataframe for csv file
    df = pd.DataFrame(index=index)
    
    #### to read template and SIFT match them
    PATH = r'../template/'
    img_names = glob(PATH+'**_mask.jpg')
    img_names.sort(key=comp_temp)
    for temp_name in img_names:
        temp_img_gray = cv2.imread(temp_name,0)
        ...
        column = []
        col_name = temp_name[len(PATH):temp_name.find('_')]+'_mask'
        for label,im in imgs:
            try:
                get_matched_coordinates(temp_img_eq, im, col_name+' '+label)
                print(count,'matched!! with label - ',label)
                column.append(1)
            except:
                print(count,'gave error :(')
                column.append(0)
            count+=1

        df[col_name] = column
        df.to_excel('results_sift_matching.xlsx')
